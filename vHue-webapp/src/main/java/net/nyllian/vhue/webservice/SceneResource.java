...
public class SceneResource
{
    ...
    public Response changeLightState(@Context HttpServletRequest request, @PathParam("id") String id, @PathParam("lid") String lid)
    {
        ...
        try
        {
            ...
            // TODO: Check which colorMode is passed (ct, xy, ...?) and set the colorMode accordingly
            Map<String, String> postMap = Serializer.SerializeJson(postData, Map.class);
            if (postMap.containsKey("ct"))
            {
                updatedSceneLightState.setColorMode("ct");
            }
            else if (postMap.containsKey("xy"))
            {
                updatedSceneLightState.setColorMode("xy");
            }
            else
            {
